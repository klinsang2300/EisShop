...
export const ProductBts = async (): Promise<Product[]> => {
    const getdataDetail: ProductDetail = ProductDetaildata();
    const mockData: Product[] = Array.from({ length: 4 }, (_, i) => (getdataDetail[`bts${i + 1}`]))
    return mockData;
}
export const ProductTwm = async (): Promise<Product[]> => {
    ...
    { src: tmw4, nameProduct: 'OFFICIAL LIGHT STICK STAND', price: '1490', Remark: 'Shipped from Korea to Thailand' },
    ];
}
export const ProductEn = async (): Promise<Product[]> => {
    ...
    { src: en4, nameProduct: 'S/S T-shirt (MINE)', price: '1490', Remark: 'Shipped from Korea to Thailand' },
    ];
}

export const ProductSt = async (): Promise<Product[]> => {
    return [
        { src: ST1, nameProduct: 'SEVENTEEN UNIT \nPHOTOBOOK ‘EPISODE C’', price: '1080', Remark: 'Shipped from Korea to Thailand' },
        ...
        { src: ST4, nameProduct: 'NANA bnb with SEVENTEEN 2025 KIT', price: '1490', Remark: 'Shipped from Korea to Thailand' },
    ];
}
...
export const ProductHot = async (): Promise<Product[]> => {
    const getdataDetail: ProductDetail = ProductDetaildata();
    const mockData: Product[] = Array.from({ length: 8 }, (_, i) => (getdataDetail[`hot${i + 1}`]));
    return mockData;
}
